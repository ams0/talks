...
|---|---|---|---|
| 1 | Title | No bio. First line is a question — next slide. | 0:00 |
| 2 | Who's in the room? | Four hands-up: owner / team lead / solo / >20 people. Picks the examples for the rest of the talk. | 0:20 |
| 3 | Where are you with AI? | Three hands-up, hands stay up: used it → it runs weekly → can name the euros. The drop opens the gap. | 1:10 |
| 4 | Restaurants, same supplier | Anchor analogy: ingredients vs. kitchen. Returns in every section. | 2:10 |
| 5 | McKinsey bars 88 → 33 → 39 → 6 | The gap in numbers. Ends on "what the six do differently". | 3:30 |
| 6 | "Access isn't the advantage. Decisions are." | Thesis #1. | 5:00 |
| 7 | What you leave with | The three outputs: X-rayed process, working teammate, review date. Each is a box on the worksheet. | 5:30 |
| 8 | How the next forty minutes run | Time blocks; the yellow one is theirs. Phones out, pens, pair up. | 6:15 |
| 9 | Section: The three calls | | 6:45 |
| 10 | Three calls overview | Build on top · keep · review. One line each. | 7:15 |
| 11 | Call 1 — dishwasher in the garage | Beside vs. inside the work. Question to the room. | 8:15 |
| 12 | Call 2 — autopilot / pilot | Four things you keep human. Question to the room. | 9:45 |
| 13 | Call 3 — experiment loop | Hypothesis → 2-week test → keep/fix/kill. Chemist line. | 11:15 |
| 14 | Section: Luca's kitchen | Optional 90-second live interview. | 12:45 |
| 15 | Luca's story in five beats | **Placeholder.** Before / built / kept / dropped / after. | 13:45 |
| 16 | The flow with one human checkpoint | **Illustrative.** Points at the coral box = Call 2 made concrete. | 16:15 |
...
1. **Worksheets on the tables before people sit down.** Done in the script and on slide 8 ("the worksheet is on your table"). Saves the shuffle at slide 17.
2. **Exercise 1 to 6:00** (2 solo · 3 pairs · 1 shares). X-raying one process in 5–8 steps doesn't need eight minutes; the pairs phase is where the value is and three minutes is enough for one challenge. Change `data-timer="480"` → `360` on slide 18 and the phase strip, the "8:00" on slide 8, and the handout's "8 minutes".
...
- **Slides 15 and 16 are the single point of failure.** Six minutes of the talk are placeholders. Add a fallback to the script: if Luca's story isn't confirmed by the day, drop slide 15, keep slide 16 relabelled "a typical pattern from our community", and give the two minutes back to Exercise 1.
